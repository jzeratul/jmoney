...
//	private final JMoneyEncryptionService jasyptEncryptionService;
	...
	public ResponseEntity<WebUser> login(@Valid WebUser webUser) {
		...
		final JMoneyUser userDetails = (JMoneyUser) jwtService.loadUserByUsername(webUser.getUsername());

		final String token = jwtToken.generateToken(userDetails);

		final WebUser loggedUser = new WebUser().username(webUser.getUsername()).jwtToken(token);
	...
	private void authenticate(WebUser webUser) {
		try {
			authenticationManager
									.authenticate(new UsernamePasswordAuthenticationToken(webUser.getUsername(), webUser.getPassword()));
